...
export default function HomePage() {
  ...
  return (
    <main className="grid min-h-screen place-items-center p-6">
      <section className="w-full max-w-[848px] rounded-[20px] border-2 border-black bg-[color-mix(in_hsl,var(--color-panel)_88%,white_12%)] p-6 shadow-[8px_8px_0_#000] animate-card-in">
        <h1
        ...
        <p className="mt-2 mb-6 text-base">
          Type a GitHub username. We inspect recent public PRs and judge whether your code habits
          are disciplined or absolute chaos.
        </p>

        <form className="flex flex-wrap gap-3" onSubmit={onSubmit}>
          <input
            className="min-w-[260px] flex-1 rounded-full border-2 border-black bg-white px-4 py-3"
            type="text"
            value={username}
          ...
          />
          <button
            className="cursor-pointer rounded-full border-2 border-black bg-accent px-6 py-3 font-bold text-white disabled:cursor-not-allowed disabled:opacity-60"
            type="submit"
            disabled={loading}
          >
        ...
        {error ? <div className="mt-3 font-semibold text-[#8a0d0d]">{error}</div> : null}
          ...
          <>
            ...
            <div className="mt-5 grid grid-cols-[repeat(auto-fit,minmax(152px,1fr))] gap-3">
              {orderedBuckets.map((bucket) => (
                <div
                  className="rounded-xl border-2 border-black bg-white p-3 [animation:card-in_460ms_ease-out_both]"
                  key={bucket}
                >
            ...
            <div className="mt-6 max-h-[240px] overflow-auto rounded-xl border-2 border-black bg-white p-2">
              {audit.prs.slice(0, 20).map((pr) => (
                <article className="rounded-lg border-t border-t-[#d8d8d8] p-2 first:border-t-0" key={pr.url}>
